...
public class FlightDAO extends BaseDAO<Flight> {
	...
	public void create(Flight flight) throws ClassNotFoundException, SQLException {
		save("insert into flight(id,route_id,airplane_id,departure_time,arrival_time,"
				+ "reserved_seats,reserved_seats2,reserved_seats3,seat_price) values(?,?,?,?,?,?,?,?,?)",
				new Object[] { flight.getId(), flight.getRouteId(), flight.getAirplaneId(), flight.getDeparture(),
						flight.getArrival(), flight.getReservedSeats(), flight.getReservedSeats2(),
						flight.getReservedSeats3(), flight.getSeatPrice() });
	}
	...
	public void update(Flight flight) throws ClassNotFoundException, SQLException {
		save("update flight set route_id=?, airplane_id=?,arrival_time=?, departure_time=?,reserved_seats=?,reserved_seats2=?,reserved_seats3=?,"
				+ "seat_price=? where id=?",
				new Object[] { flight.getRouteId(), flight.getAirplaneId(), flight.getArrival(), flight.getDeparture(),
						flight.getReservedSeats(), flight.getReservedSeats2(), flight.getReservedSeats3(),
