...
public class PassengerDAO extends BaseDAO<Passenger> {
	...
	public void create(Passenger p) throws ClassNotFoundException, SQLException {
		save("insert into passenger(booking_id,given_name,family_name,dob,gender,address) values(?,?,?,?,?,?)",
				new Object[] { p.getBookingId(), p.getGivenName(), p.getFamilyName(), p.getDob(), p.getGender(),
						p.getAddress() });
	}
	...
	protected List<Passenger> extractData(ResultSet rs) throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			array.add(new Passenger(rs.getInt("id"), rs.getInt("booking_id"), rs.getString("given_name"),
					rs.getString("family_name"), rs.getDate("dob").toLocalDate(), rs.getString("gender"),
